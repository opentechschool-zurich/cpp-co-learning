...
void Arc::draw_lines() const
{
	if (fill_color().visibility()) {	// fill
		fl_color(fill_color().as_int());
		fl_pie(point(0).x,point(0).y,mRadius+mRadius-1,mRadius+mRadius-1,mStartAngle,mEndAngle);
		fl_color(color().as_int());	// reset color
	}

	if (color().visibility()) {
		fl_color(color().as_int());
		fl_arc(point(0).x,point(0).y,mRadius+mRadius,mRadius+mRadius,mStartAngle,mEndAngle);
	}
}
