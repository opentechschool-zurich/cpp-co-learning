...
public:
    ...
    int parseLinks()
    {
        ...
        if (htmlDocument) {
            ...
            count = collectLinks(root_element);
        }

        xmlFreeDoc(htmlDocument); // clear memory
        std::cout << count << " links added to node " << node << std::endl;
        return count;
    }
     ...
     * @return The number of links added
     */
    int collectLinks(xmlNode *a_node)
    {
        ...
        for (cur_node = a_node; cur_node; cur_node = cur_node->next) {
            ...
            count += collectLinks(cur_node->children);
        }
        return count;
    }
    ...
    /**
     * @brief Returns the link corresponding to the searchNumber
     * @param searchNumber the number to search for
     * @return an optional<Link> with the link found
     */
    std::optional<Link *> findLink(int searchNumber)
    {
        if (node == searchNumber) {
            std::cout << "Found " << searchNumber << " on node " << node << std::endl;
            // std::cout << p->toString();
            return this;
        } else {
            std::cout << "Didn't find " << searchNumber << " on node " << node << std::endl;
            for (Link & c : children) {
                auto o = c.findLink(searchNumber);
                if (o) {
                    // std::cout << o->toString();
                    return o;
                }
